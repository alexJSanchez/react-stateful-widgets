...
let [pioneers, setPioneers] = useState(listOfAwesome);
let [idFeatured, setIdFeatured] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    const foundDev = pioneers.find(dev => dev.id === idFeatured);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idFeatured ? 'gold' : 'royalblue', 
      ...
      <div className='programmers'>
        {
         ...
         pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIdFeatured(dev.id)  }}>Feature</button>
            </div>
          )
        }
      </div>
